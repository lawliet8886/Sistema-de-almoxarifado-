...
class MainWindow(QMainWindow):
    ...
    def search_products(self):
        ...
        if selected_category == "Todas as Categorias":
            c.execute("SELECT nome, categoria, quantidade, quantidade_minima, vencimento, retirada, data_registro FROM produtos WHERE nome LIKE ?", (f'%{search_text}%',))
        else:
            c.execute("SELECT nome, categoria, quantidade, quantidade_minima, vencimento, retirada, data_registro FROM produtos WHERE nome LIKE ? AND categoria=?", (f'%{search_text}%', selected_category))
